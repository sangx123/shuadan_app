...
public class RefundPlatformListAdapter extends RecyclerView.Adapter {
    ...
    public RefundPlatformListAdapter(Activity activity, ArrayList<RefundViewItemlModel> items, IRecycleViewCallBack iRecycleViewCallBack) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == TYPE_HEAD) {
            View itemView = LayoutInflater.from(activity).inflate(R.layout.header_refund_platform_layout, parent, false);
            return new RefundPlatformListAdapter.HeadHolder(itemView);
        } else if (viewType == TYPE_CONTENT) {
            View itemView = LayoutInflater.from(activity).inflate(R.layout.item_platfrom_layout, parent, false);
            return new RefundPlatformListAdapter.ItemViewHolder(itemView);
        } else {
            View itemView = LayoutInflater.from(activity).inflate(R.layout.bottom_platform_layout, parent, false);
            return new RefundPlatformListAdapter.FootHolder(itemView);
        }
    }

    @Override
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof RefundPlatformListAdapter.HeadHolder) { // 头部

            HeadHolder myHolder = (HeadHolder) holder;

        } else if (holder instanceof RefundPlatformListAdapter.ItemViewHolder) { // 内容
        ...
        } else if (holder instanceof RefundPlatformListAdapter.FootHolder) { // 尾部
    ...
    private class FootHolder extends RecyclerView.ViewHolder {
        ...
        public FootHolder(View itemView) {
            ...
            toApplyLoanToBackMoney.setText(activity.getString(R.string.upload_picture));
        }
    }
