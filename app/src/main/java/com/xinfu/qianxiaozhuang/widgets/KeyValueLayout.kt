...
class KeyValueLayout(context: Context, attrs: AttributeSet?, defStyleAttr: Int) : RelativeLayout(context, attrs, defStyleAttr) {
    ...
    init {
        ...
        if (attrs != null) {
            ...
            if (obtainStyledAttributes.hasValue(R.styleable.KeyValueLayout_kvl_has_line)) {
                val has = obtainStyledAttributes.getBoolean(R.styleable.KeyValueLayout_kvl_has_line, false)
                if(!has) line1.visibility= View.GONE
            }

            if (obtainStyledAttributes.hasValue(R.styleable.KeyValueLayout_kvl_left_icon_width)) {
                val width = obtainStyledAttributes.getDimension(R.styleable.KeyValueLayout_kvl_left_icon_width, 0f)
                val height = obtainStyledAttributes.getDimension(R.styleable.KeyValueLayout_kvl_left_icon_height, 0f)
                var params = leftIcon.layoutParams
                params.width = width.toInt()
                params.height = height.toInt()
                leftIcon.layoutParams = params
            }
            if (obtainStyledAttributes.hasValue(R.styleable.KeyValueLayout_kvl_left_icon)) {
                val drawable = obtainStyledAttributes.getDrawable(R.styleable.KeyValueLayout_kvl_left_icon)
                leftIcon.setImageDrawable(drawable)
            }



            if (obtainStyledAttributes.hasValue(R.styleable.KeyValueLayout_kvl_left_with)) {
                val width = obtainStyledAttributes.getDimension(R.styleable.KeyValueLayout_kvl_left_with, 0f)
                var params = tv_title_layout.layoutParams
                params.width = (width-DisplayUtil.getInstance().with(getContext()).dpToPixel(15f)).toInt()
                tv_title_layout.layoutParams = params
            }
